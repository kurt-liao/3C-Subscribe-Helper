...
class MYSQL_Helper:
    ...
    def check_subDB(self):
        stmt = "CREATE TABLE IF NOT EXISTS subDB (username text,product text,price text,type text,duetime timestamp,is_LineSub int,isInSql INTEGER)"
        self.conn.execute(stmt)
        self.conn.commit()
    ...
    def add_sub(self, username, product, price, type, duetime,is_LineSub):
        ...
        self.conn.commit()
    #刪除訂閱
    def delete_sub(self, username, product, price,type):
        stmt = "DELETE FROM subDB WHERE username = (?) and product=(?) and price=(?) and type=(?)"
        args = (username, product, price, type,)
        self.conn.execute(stmt, args)
        self.conn.commit()
    #目前沒用到
